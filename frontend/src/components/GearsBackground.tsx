...
const gearTypes = ['standard', 'industrial', 'small', 'large', 'sprocket'] as const;
const appIconTypes = ['settings', 'tools', 'dashboard', 'widget', 'menu'] as const;

type GearType = typeof gearTypes[number];
type AppIconType = typeof appIconTypes[number];
type AnimationStyle = 'rotating' | 'floating' | 'transforming';
...
export default function GearsBackground() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < itemCount; i++) {
            ...
            const appIconType = appIconTypes[Math.floor(Math.random() * appIconTypes.length)];
            const animationStyles: AnimationStyle[] = ['rotating', 'floating', 'transforming'];
    ...
    function updateVisual(item: ToolItem) {
        if (item.isAppIcon) {
            item.element.className = `tool-item app-icon ${item.appIconType} ${item.animationStyle}`;
            item.element.innerHTML = appIconSVGs[item.appIconType];
        } else {
            item.element.className = `tool-item gear ${item.gearType} ${item.animationStyle}`;
            item.element.innerHTML = gearSVGs[item.gearType];
        }
        item.element.style.transform = `scale(${item.scale})`;
    }

    function updatePosition(item: ToolItem) {
        item.element.style.left = `${item.x}px`;
        item.element.style.top = `${item.y}px`;
    }

    function updateItem(item: ToolItem, deltaTime: number) {
        // Move across screen
        item.x += item.vx;
        item.y += item.vy;

        // Wrap around edges
        if (item.x < -500) item.x = window.innerWidth + 200;
        if (item.x > window.innerWidth + 200) item.x = -500;
        if (item.y < -500) item.y = window.innerHeight + 200;
        if (item.y > window.innerHeight + 200) item.y = -500;

        updatePosition(item);

        // Transformation logic
        item.transformationTimer += deltaTime;

        if (item.transformationTimer >= item.transformationDuration) {
            item.element.classList.add('morphing');

            setTimeout(() => {
                item.isAppIcon = !item.isAppIcon;

                if (item.isAppIcon) {
                    item.appIconType = appIconTypes[Math.floor(Math.random() * appIconTypes.length)];
                } else {
                    item.gearType = gearTypes[Math.floor(Math.random() * gearTypes.length)];
                }

                if (Math.random() > 0.6) {
                    const animationStyles: AnimationStyle[] = ['rotating', 'floating', 'transforming'];
                    item.animationStyle = animationStyles[Math.floor(Math.random() * animationStyles.length)];
                }

                item.scale = 0.7 + Math.random() * 0.6;
                updateVisual(item);

                setTimeout(() => {
                    item.element.classList.remove('morphing');
                }, 300);
            }, 200);

            item.transformationTimer = 0;
            item.transformationDuration = 5000 + Math.random() * 5000;
        }
    }

    return (
        <div className="gears-background absolute inset-0 overflow-hidden pointer-events-none z-0">
            <div 
                ref={containerRef}
                className="items-container absolute inset-0"
            />
        </div>
    );
}
